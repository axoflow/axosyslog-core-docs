...
The `stomp()` driver sends messages to servers (message brokers) using the [Simple (or Streaming) Text Oriented Message Protocol (STOMP)](http://stomp.github.io/), formerly known as TTMP. {{% productparam "abbrev" %}} supports version 1.0 of the STOMP protocol. The {{% productparam "abbrev" %}} `stomp()` driver supports persistence.
