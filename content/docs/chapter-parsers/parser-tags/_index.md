...
The {{% param "product.name" %}} ({{% param "product.abbrev" %}}) application can tag log messages, and can include these tags in the log messages, as described in {{% xref "/docs/chapter-routing-filters/filters/tagging-messages/_index.md" %}}. The `tags-parser()` can parse these tags from the incoming messages and re-tag them. That way if you add tags to a log message on a {{% param "product.abbrev" %}} client, the message will have the same tags on the {{% param "product.abbrev" %}} server. Available in version {{% conditional-text include-if="pe" %}}7.0.16{{% /conditional-text %}}{{% conditional-text include-if="ose" %}}3.23{{% /conditional-text %}} and later.
