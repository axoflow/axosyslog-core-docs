...
This section is a brief overview on how {{% productparam "abbrev" %}} works in multithreaded mode. It is mainly for illustration purposes: the concept has been somewhat simplified and may not completely match reality.

{{% alert title="Note" color="info" %}}

The way {{% productparam "abbrev" %}} uses multithreading may change in future releases. The current documentation applies to version {{% productparam "version" %}}.

{{% /alert %}}

{{% productparam "abbrev" %}} always uses multiple threads:
  ...
  - Some other, special threads for internal functionalities. For example, certain destinations run in a separate thread, independently of the multithreading (`threaded()`) and `--worker-threads` settings of {{% productparam "abbrev" %}}.

The maximum number of worker threads {{% productparam "abbrev" %}} uses is the number of CPUs or cores in the host running {{% productparam "abbrev" %}} (up to 64). You can limit this value using the **--worker-threads** command-line option that sets the maximum total number of threads {{% productparam "abbrev" %}} can use, including the main {{% productparam "abbrev" %}} thread. However, the `--worker-threads` option does not affect the supervisor of {{% productparam "abbrev" %}}. The supervisor is a separate process (see <span class="mcFormatColor" style="color: #04aada;">The syslog-ng manual page</span>), but certain operating systems might display it as a thread. In addition, certain destinations always run in a separate thread, independently of the multithreading (`threaded()`) and `--worker-threads` settings of {{% productparam "abbrev" %}}.

When an event requiring a new thread occurs (for example, {{% productparam "abbrev" %}} receives new messages, or a destination becomes available), {{% productparam "abbrev" %}} tries to start a new thread. If there are no free threads, the task waits until a thread finishes its task and becomes available. There are two types of worker threads:
...
The following list describes which sources and destinations can use multiple threads. Changing the `--worker-threads` command-line option changes the number of threads available to these sources and destinations.
  ...
  - The `tcp` and `syslog(tcp)` sources can process independent connections in separate threads. The number of independent connections is limited by the `max-connections()` option of the source. Separate sources are processed by separate thread, for example, if you have two separate `tcp` sources defined that receive messages on different IP addresses or port, {{% productparam "abbrev" %}} will use separate threads for these sources even if they both have only a single active connection.
...
The following list describes sources and destinations that use a separate thread even if you disable multithreading in {{% productparam "abbrev" %}}, in addition to the limit set in the `--worker-threads` command-line option.
