...
This section describes a round robin load balancing method based on the `R_MSEC` macro of {{% productparam "name" %}} ({{% productparam "abbrev" %}}) to load balance your logs between multiple {{% productparam "abbrev" %}} destinations.
...
The following example is a round-robin load balancing method, based on {{% productparam "abbrev" %}}'s `R_MSEC` macro.
...
If you need a file instead of a network destination, replace the network destination with the file in the example (and use the same analogy for any other {{% productparam "abbrev" %}} destinations).
