...
1. Configure the network sources that collect the log messages sent by the clients.
1. Create a network destination that points to the AxoSyslog server.
...
1. Create a log statement connecting the network sources to the AxoSyslog server.
1. Configure the local sources that collect the log messages of the relay host.
1. Create a log statement connecting the local sources to the AxoSyslog server.
1. Enable the `keep-hostname()` and disable the `chain-hostnames()` options. (For details on how these options work, see [Global options]({{< relref "/chapter-global-options/reference-options/_index.md" >}}).)

    {{% alert title="Note" color="info" %}}
It is recommended to use these options on your {{% param "product.abbrev" %}} server as well.
    {{% /alert %}}

1. Set filters and options (for example, TLS encryption) as necessary.
