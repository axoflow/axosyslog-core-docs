...
The `graphite()` destination is a reusable configuration snippet based on the `network()` destination, so you can use the [options of the `network()` destination]({{< relref "/chapter-destinations/configuring-destinations-network/reference-destination-network-chapter/_index.md" >}}) as well. You can find the source of this configuration snippet on [GitHub](https://github.com/axoflow/axosyslog/blob/master/scl/graphite/plugin.conf). Note that the `graphite()` destination changes the default value of the following `network()` option.
...
*Description:* The port number of the Graphite server. The `graphite()` destination changes the default value of the underlying `network()` destination from `601` to `2003`, the port that the Graphite plaintext protocol listens on.
