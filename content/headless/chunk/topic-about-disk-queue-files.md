...
Both reliable and normal disk-buffers employ an in-memory output queue (set in `front-cache-size()`) and an in-memory overflow queue (set in `flow-control-window-bytes()` for reliable disk-buffers, or `flow-control-window-size()` for normal disk-buffers). The difference between reliable and normal disk-buffers is that when the reliable disk-buffer uses one of its in-memory queues, it also stores the message on the disk, whereas the normal disk-buffer stores the message only in memory. The normal disk-buffer only uses the disk if the in-memory output buffer is filled up completely. This approach has better performance (due to fewer disk I/O operations), but also carries the risk of losing a maximum of `front-cache-size()` plus `flow-control-window-size()` number of messages in case of an unexpected power failure or application crash.
